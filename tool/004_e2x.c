...
char discEdge(char p,char s,char dir)
{
	...
	if(board[s+dir]==p && board[s+dir+dir]==p && board[s+dir*3]==p && board[s+dir*4]==p && board[s+dir*5]==3-p) return 20;
	if(board[s+dir]==p && board[s+dir+dir]==p && board[s+dir*3]==p && board[s+dir*4]==p && board[s+dir*5]==p && board[s+dir*6]==3-p) return 20;
...
char corner(char s,char dir1,char dir2)
{
	...
	if(board[s+dir1]==1 && board[s+dir1+dir1]==1 && board[s+dir1*3]==1) return 8;
	if(board[s+dir1]==2 && board[s+dir1+dir1]==2 && board[s+dir1*3]==2) return -8;
